...
xstring_t
xstring_from_cstr(const char *cstr, see_uintptr_t len)
{
    xstring_t result = (xstring_t)SEE_MALLOC(sizeof(struct xstring_s));
    if (result == NULL) return NULL;

    if (len == XSTRING_LEN_UNDEFINED) len = SEE_STRLEN(cstr);
...
int
xstring_equal_cstr(xstring_t string, const char *cstr, see_uintptr_t len)
{
    if (len == XSTRING_LEN_UNDEFINED) len = SEE_STRLEN(cstr);
